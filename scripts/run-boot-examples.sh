...
echo "[2/3] Start provider (requires Nacos at 127.0.0.1:8848)"
nohup java -jar "$PROVIDER_JAR" > /tmp/aoaojiao-rpc-provider.log 2>&1 &
PROVIDER_PID=$!
...
echo "[3/3] Start consumer"
nohup java -jar "$CONSUMER_JAR" > /tmp/aoaojiao-rpc-consumer.log 2>&1 &
CONSUMER_PID=$!
...
echo "Logs: /tmp/aoaojiao-rpc-provider.log /tmp/aoaojiao-rpc-consumer.log"
